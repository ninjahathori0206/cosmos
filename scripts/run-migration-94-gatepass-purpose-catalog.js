/**
 * Migration 94 — gatepass_purpose_catalog
 * Usage: npm run migrate:94-gatepass-purpose-catalog
 */
...
async function main () {
  ...
  const migFile = path.join(__dirname, '..', 'sql', 'migrations', '94_gatepass_purpose_catalog.sql')
...
main().catch((err) => {
  console.error('[migration-94]', err.message || err)
  process.exit(1)
})
