...
const roleSchema = Joi.object({
  role_key:     Joi.string().max(50).pattern(/^[a-z0-9_]+$/).required()
    .messages({ 'string.pattern.base': 'role_key may only contain lowercase letters, digits, and underscores (no hyphens or spaces).' }),
...
router.post(
  '/',
  requireModule('command_unit'),
  requirePermission('command_unit.roles.create'),
  async (req, res, next) => {
    try {
...
router.put(
  '/:key',
  requireModule('command_unit'),
  requirePermission('command_unit.roles.edit'),
  async (req, res, next) => {
    try {
