...
-- Add per-colour image URL to purchase_item_colours (set during Digitisation)
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='purchase_item_colours' AND COLUMN_NAME='image_url')
  ALTER TABLE dbo.purchase_item_colours ADD image_url VARCHAR(500) NULL;

-- Add per-SKU image URL to skus (copied from purchase_item_colours at SKU generation)
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='skus' AND COLUMN_NAME='image_url')
  ALTER TABLE dbo.skus ADD image_url VARCHAR(500) NULL;

GO

-- ═══ sp_PurchaseItemColour_SetImage — saves image URL for a colour variant ═══
IF OBJECT_ID('dbo.sp_PurchaseItemColour_SetImage','P') IS NOT NULL
  DROP PROCEDURE dbo.sp_PurchaseItemColour_SetImage;
GO
CREATE PROCEDURE dbo.sp_PurchaseItemColour_SetImage
  @colour_id INT,
...
AS BEGIN
  ...
  BEGIN TRY
    ...
    SELECT colour_id, colour_name, colour_code, image_url
...
-- ═══ sp_SKUv2_Generate — updated to carry image_url from purchase_item_colours ═══
...
AS BEGIN
  ...
  BEGIN TRY
    ...
    DECLARE @brand_name VARCHAR(200), @colour_image_url VARCHAR(500);
    ...
    -- Build SKU code: BRAND-COLLECTION-COLOUR-UNIQUE
...
-- ═══ sp_PurchaseHeader_GetById — include image_url from purchase_item_colours in RS2 ═══
...
AS BEGIN
  ...
  -- RS1: items (includes product detail fields)
  ...
  ORDER BY pi.item_id;
  -- RS2: colours (includes per-colour image_url)
...
-- ═══ sp_PurchaseHeader_GetSKUs — include image_url from skus ═══
...
AS BEGIN
  ...
  SELECT
    ...
    sk.status, sk.item_colour_id, sk.image_url,
...
-- ═══ sp_SKU_GetAll — include per-SKU image_url (prefer skus.image_url, fallback to product) ═══
