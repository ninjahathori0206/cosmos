...
OUTER APPLY (
  SELECT
    ...
    ISNULL(SUM(o.total_amount), 0) AS booking_revenue,
    CASE WHEN COUNT(*) > 0 THEN ISNULL(SUM(o.total_amount), 0) / COUNT(*) ELSE 0 END AS avg_booking_amount
...
OUTER APPLY (
  SELECT COUNT(*) AS memberships_sold
  FROM ' + @orders_table + N' o
  WHERE o.store_id = @store_id
    AND CONVERT(DATE, o.created_at) = @report_date
    AND ISNULL(o.status, N'''') <> N''CANCELLED''
    AND o.sold_membership_plan_key IS NOT NULL
...
';
  ...
  EXEC sp_executesql @sql,
    N'@store_id INT, @report_date DATE',
    @store_id = @store_id,
