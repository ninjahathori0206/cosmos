/*
  GatePass — auto-assign visitor to staff who performed check-in.
  Sets assigned_user_id = checkin_by_user_id on new check-in and same-day reopen.
*/
USE [CosmosERP];
GO

-- Backfill active queue rows checked in by staff but never assigned
UPDATE sv
SET assigned_user_id = sv.checkin_by_user_id,
    updated_at = DATEADD(MINUTE, 330, SYSUTCDATETIME())
FROM dbo.store_visitors sv
WHERE sv.assigned_user_id IS NULL
  AND sv.checkin_by_user_id IS NOT NULL
  ...
  BEGIN TRY
    ...
    BEGIN
      ...
      SET name = @name,
          ...
          assigned_user_id = CASE
            WHEN @checkin_by_user_id IS NOT NULL THEN @checkin_by_user_id
    ...
    BEGIN
      INSERT INTO dbo.store_visitors (
        name, phone, store_id, customer_id, purpose, checkin_at, checkin_channel,
        checkin_by_user_id, assigned_user_id,
        status, status_changed_at, expiry_at, notes, created_at, updated_at
      )
      VALUES (
        @name, @phone, @store_id, @customer_id, @purpose, @now, @channel,
        @checkin_by_user_id, @checkin_by_user_id,
...
-- Queue list: expose assigned_user_id for API mapper
...
PRINT 'GatePass check-in auto-assign migration complete.';
GO
